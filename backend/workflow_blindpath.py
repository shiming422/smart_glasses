...
def _build_seg_predict_kwargs(conf_threshold: float, classes: Optional[List[int]] = None) -> Dict[str, Any]:
    kwargs: Dict[str, Any] = {
...
class BlindPathNavigator:
    ...
    def __init__(self, yolo_model=None, obstacle_detector=None):
        ...
        self.CLASS_CONF_THRESHOLDS = {
            1: _env_float("AIGLASS_BP_CONF", 0.20),   # blind_path
            0: _env_float("AIGLASS_CW_CONF", 0.30),   # crosswalk
        }
        ...
        # 斑马线切换阈值
        self.CROSSWALK_SWITCH_AREA_RATIO = _env_float("AIGLASS_CW_SWITCH_AREA", 0.22)
        self.CROSSWALK_SWITCH_BOTTOM_RATIO = _env_float("AIGLASS_CW_SWITCH_BOTTOM", 0.9)
        self.CROSSWALK_SWITCH_CONSECUTIVE_FRAMES = _env_int("AIGLASS_CW_SWITCH_CONSEC", 10)
        ...
        self.MASK_MISS_TTL = _env_int("AIGLASS_MASK_MISS_TTL", 0)  # 0=禁用光流外推（实时），>0=帧数保活
        ...
        # 盲道/斑马线分割降频：跳过的帧复用上一轮掩码，优先保证主 viewer 连续出图。
        self.BLINDPATH_DETECTION_INTERVAL = max(
            1,
            _env_int(
                "AIGLASS_PATH_FRAME_DIV",
                _env_int("AIGLASS_BLINDPATH_INTERVAL", 1 if SEG_ACCURACY_MODE else 2),
            ),
        )
        ...
        logger.info(
            "[BlindPath] seg cfg: accuracy=%s imgsz=%s iou=%s augment=%s half=%s retina_masks=%s",
            SEG_ACCURACY_MODE,
    ...
    def _detect_path_and_crosswalk(self, image: np.ndarray) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
        ...
        if (
            self.last_blindpath_detection_frame > 0
            and (self.frame_counter - self.last_blindpath_detection_frame) < self.BLINDPATH_DETECTION_INTERVAL
        ):
            cached_blind = self.last_blindpath_mask.copy() if self.last_blindpath_mask is not None else None
            cached_crosswalk = (
                self.last_crosswalk_mask.copy() if self.last_crosswalk_mask is not None else None
            )
            return cached_blind, cached_crosswalk
        
        try:
            min_conf = min(self.CLASS_CONF_THRESHOLDS.values())
            predict_kwargs = _build_seg_predict_kwargs(min_conf, classes=[0, 1])
            results = self.yolo_model.predict(image, **predict_kwargs)
            ...
            self.last_blindpath_detection_frame = self.frame_counter
            self.last_blindpath_mask = blind_path_mask.copy() if blind_path_mask is not None else None
            self.last_crosswalk_mask = crosswalk_mask.copy() if crosswalk_mask is not None else None
        except Exception as e:
            ...
            blind_path_mask[int(h*0.3):, strip_left:strip_left+strip_width] = 255
            self.last_blindpath_detection_frame = self.frame_counter
            self.last_blindpath_mask = blind_path_mask.copy()
            self.last_crosswalk_mask = None
    ...
    def reset(self, start_state: str = STATE_ONBOARDING):
        """重置导航器状态，可选直接进入常规导航态。"""
        if start_state not in {
            STATE_ONBOARDING,
            STATE_NAVIGATING,
            STATE_MANEUVERING_TURN,
            STATE_AVOIDING_OBSTACLE,
            STATE_LOCKING_ON,
        }:
            start_state = STATE_ONBOARDING
    ...
    def start_navigation_session(self):
        """启动新的盲道导航会话，跳过初始 onboarding 校准。"""
        self.reset(start_state=STATE_NAVIGATING)
