...
class NavigationMaster:
    ...
    def start_blind_path_navigation(self):
        ...
        if self.blind:
            if hasattr(self.blind, "start_navigation_session"):
                self.blind.start_navigation_session()
            else:
                self.blind.reset()
    ...
    def reset(self):
        self.state = IDLE
        self._reset_session_counters()
        if trafficlight_detection is not None:
            try:
    ...
    def process_frame(self, bgr: np.ndarray) -> OrchestratorResult:
        ...
        if self.state in (BLINDPATH_NAV, SEEKING_CROSSWALK, SEEKING_NEXT_BLINDPATH, RECOVERY):
            ...
            return OrchestratorResult(
                ...
                self.state,
                {
                    ...
                    "visualizations": getattr(bres, "visualizations", []) or [],
        ...
        if self.state == WAIT_TRAFFIC_LIGHT:
            ...
            if trafficlight_detection is not None:
                try:
                    result = trafficlight_detection.process_single_frame(ann)
                    vis_image = result.get("vis_image")
                    if vis_image is not None:
                        ann = vis_image
                    major = result.get("stable_light") or "unknown"
                    meta = {
            ...
            say = ""
            if major == "go" and not in_cooldown:
                self.state = CROSSING
                self.cooldown_until = now + self.COOLDOWN_SEC
                say = "绿灯稳定，开始通行。"
            else:
