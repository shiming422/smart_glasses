...
NAV_DEBUG_ENABLED = (
    os.getenv("AIGLASS_NAV_DEBUG", os.getenv("AIGLASS_VERBOSE_NAV_LOGS", "0")) == "1"
)
if not NAV_DEBUG_ENABLED:
    # Keep per-frame INFO logs opt-in to protect real-time throughput.
    logger.setLevel(logging.WARNING)

_BASE_DIR = os.path.dirname(os.path.abspath(__file__))
_CROSSWALK_DEBUG_DIR = Path(_BASE_DIR) / "debug" / "crosswalk"
...
SEG_ACCURACY_MODE = _env_flag("AIGLASS_SEG_ACCURACY_MODE", False)
SEG_IMGSZ = max(0, _env_int("AIGLASS_SEG_IMGSZ", 768 if SEG_ACCURACY_MODE else 0))
SEG_IOU = _env_float("AIGLASS_SEG_IOU", 0.50 if SEG_ACCURACY_MODE else -1.0)
SEG_AUGMENT = _env_flag("AIGLASS_SEG_AUGMENT", False)
SEG_HALF = _env_flag("AIGLASS_SEG_HALF", SEG_ACCURACY_MODE)
SEG_RETINA_MASKS = _env_flag("AIGLASS_SEG_RETINA_MASKS", SEG_ACCURACY_MODE)


def _build_seg_predict_kwargs(conf_threshold: float) -> Dict[str, Any]:
    kwargs: Dict[str, Any] = {
        "conf": float(conf_threshold),
        "verbose": False,
    }
    if SEG_IMGSZ > 0:
        kwargs["imgsz"] = SEG_IMGSZ
    if 0.0 < SEG_IOU < 1.0:
        kwargs["iou"] = SEG_IOU
    if SEG_AUGMENT:
        kwargs["augment"] = True
    ...
    if SEG_HALF and torch.cuda.is_available():
        kwargs["half"] = True
    return kwargs
...
CROSSWALK_PREPROCESS_ENABLED = _env_flag("AIGLASS_CROSSWALK_PREPROCESS", True)
CROSSWALK_INTERVAL_SEEK = max(
    1, _env_int("AIGLASS_CROSSWALK_INTERVAL_SEEK", 1 if SEG_ACCURACY_MODE else 2)
)
CROSSWALK_INTERVAL_CROSS = max(
    1, _env_int("AIGLASS_CROSSWALK_INTERVAL_CROSS", 1 if SEG_ACCURACY_MODE else 3)
)
CROSSWALK_MIN_AREA_RATIO_SEEK = _env_float("AIGLASS_CROSSWALK_MIN_AREA_RATIO_SEEK", 0.003)
CROSSWALK_MIN_AREA_RATIO_NEAR = _env_float("AIGLASS_CROSSWALK_MIN_AREA_RATIO_NEAR", 0.008)
CROSSWALK_STABLE_WINDOW = max(3, _env_int("AIGLASS_CROSSWALK_STABLE_WINDOW", 5))
CROSSWALK_STABLE_MIN = max(1, _env_int("AIGLASS_CROSSWALK_STABLE_MIN", 3))
CROSSWALK_NEAR_SCORE_THRESHOLD = _env_float("AIGLASS_CROSSWALK_NEAR_SCORE_THRESHOLD", 2.0)
DETECTION_DEBUG_ENABLED = _env_flag("AIGLASS_DETECTION_DEBUG", False)
CROSSWALK_STABLE_IOU = _env_float("AIGLASS_CROSSWALK_STABLE_IOU", 0.25)

# 远距离对准阈值
SEEKING_ANGLE_THRESH_DEG = _env_float("AIGLASS_SEEKING_ANGLE_THRESH", 15.0)
SEEKING_OFFSET_THRESH = _env_float("AIGLASS_SEEKING_OFFSET_THRESH", 0.20)

# 盲道几何过滤阈值（_looks_like_blind_path）
BP_GEO_ANGLE_MAX = _env_float("AIGLASS_BP_GEO_ANGLE_MAX", 20.0)   # 横向条纹过滤角度上限
BP_GEO_ASPECT_MIN = _env_float("AIGLASS_BP_GEO_ASPECT_MIN", 0.52) # 高宽比最小值
BP_GEO_BOTTOM_SHARE_MAX = _env_float("AIGLASS_BP_GEO_BOTTOM_SHARE", 0.50)  # 底边窄条最大占比
BP_GEO_BOTTOM_WIDTH_MAX = _env_float("AIGLASS_BP_GEO_BOTTOM_WIDTH", 0.35)  # 底边宽度上限（相对帧宽）
BP_VALID_IOU_THR = _env_float("AIGLASS_BP_VALID_IOU", 0.40)        # 盲道与斑马线最大重叠IoU

# 红绿灯判定参数
GREEN_LIGHT_STABLE_FRAMES = max(1, _env_int("AIGLASS_GREEN_LIGHT_STABLE_FRAMES", 3))
...
def _looks_like_blind_path(bp_mask: np.ndarray, cw_mask: np.ndarray, H: int, W: int) -> bool:
    ...
    # 1) 横向条纹过滤
    if abs(angle_deg) <= BP_GEO_ANGLE_MAX:
        return False
    # 2) 形状过滤
    if aspect < BP_GEO_ASPECT_MIN:
        return False
    # 3) 与斑马线高度重叠
    ...
    if bottom.sum() > 0:
        ...
        if bottom_share > BP_GEO_BOTTOM_SHARE_MAX and (w / float(W)) < BP_GEO_BOTTOM_WIDTH_MAX:
            return False
    return True
...
class CrossStreetNavigator:
    ...
    def __init__(self, seg_model=None, coco_model=None, obs_model=None, device_id: str = "esp32"):
        ...
        # —— 斑马线检测/稳定缓存 ——
        self.crosswalk_interval_seek = CROSSWALK_INTERVAL_SEEK
        self.crosswalk_interval_cross = CROSSWALK_INTERVAL_CROSS
        self.last_crosswalk_detection_frame = 0
        self.last_detected_crosswalk_mask = None
        self.last_detected_blindpath_mask = None
        self.crosswalk_candidate_history = deque(maxlen=CROSSWALK_STABLE_WINDOW)
        self.last_crosswalk_candidate_bbox = None
        self.last_crosswalk_candidate_stable = False
        self.last_crosswalk_switch_reason = ""
        ...
        logger.info(
            "[CROSS_STREET] 斑马线检测间隔: SEEK/WAIT=%s, CROSS=%s",
            self.crosswalk_interval_seek,
            self.crosswalk_interval_cross,
        )
        logger.info(
            "[CROSS_STREET] seg cfg: accuracy=%s imgsz=%s iou=%s augment=%s half=%s retina_masks=%s",
            SEG_ACCURACY_MODE,
            SEG_IMGSZ or "default",
            SEG_IOU if 0.0 < SEG_IOU < 1.0 else "default",
            SEG_AUGMENT,
            bool(SEG_HALF and torch.cuda.is_available()),
            SEG_RETINA_MASKS,
        )
    ...
    def reset(self):
        ...
        self.last_detected_crosswalk_mask = None
        self.last_detected_blindpath_mask = None
        self.crosswalk_candidate_history.clear()
        self.last_crosswalk_candidate_bbox = None
        self.last_crosswalk_candidate_stable = False
        self.last_crosswalk_switch_reason = ""
        # 重置红绿灯检测状态
    ...
    def _current_crosswalk_detection_interval(self) -> int:
        if self.state == STATE_CROSSING:
            return self.crosswalk_interval_cross
        return self.crosswalk_interval_seek
    ...
    def _preprocess_crosswalk_input(self, image: np.ndarray) -> np.ndarray:
        if image is None or image.size == 0 or not CROSSWALK_PREPROCESS_ENABLED:
            return image
        lab = cv2.cvtColor(image, cv2.COLOR_BGR2LAB)
        l, a, b = cv2.split(lab)
        clahe = cv2.createCLAHE(clipLimit=2.2, tileGridSize=(8, 8))
        l = clahe.apply(l)
        restored = cv2.cvtColor(cv2.merge((l, a, b)), cv2.COLOR_LAB2BGR).astype(np.float32)
        restored = np.clip(np.power(np.clip(restored / 255.0, 0.0, 1.0), 0.92) * 255.0, 0, 255)
        sharpen = np.array([[0, -1, 0], [-1, 5.2, -1], [0, -1, 0]], dtype=np.float32)
        restored = cv2.filter2D(restored, -1, sharpen)
        return np.clip(restored, 0, 255).astype(np.uint8)

    def _compute_crosswalk_min_area(self, h: int, w: int) -> int:
        total = float(h * w)
        if self.state == STATE_SEEKING:
            return max(int(total * CROSSWALK_MIN_AREA_RATIO_SEEK), 1800)
        return max(int(total * CROSSWALK_MIN_AREA_RATIO_NEAR), 3200)

    def _record_crosswalk_candidate(self, mask: Optional[np.ndarray]) -> bool:
        bbox = self._mask_bbox(mask)
        self.crosswalk_candidate_history.append(bbox)
        self.last_crosswalk_candidate_bbox = bbox
        stable = self._is_crosswalk_candidate_stable(bbox)
        self.last_crosswalk_candidate_stable = stable
        return stable

    def _is_crosswalk_candidate_stable(self, bbox: Optional[tuple[int, int, int, int]]) -> bool:
        if bbox is None:
            return False
        support = sum(
            1
            for candidate in self.crosswalk_candidate_history
            if candidate is not None and self._bbox_iou(candidate, bbox) >= CROSSWALK_STABLE_IOU
        )
        return support >= CROSSWALK_STABLE_MIN

    def _save_crosswalk_debug(self, image: np.ndarray, mask: Optional[np.ndarray], blindpath_mask: Optional[np.ndarray], reason: str) -> None:
        if not DETECTION_DEBUG_ENABLED or image is None or image.size == 0:
            return
        try:
            _CROSSWALK_DEBUG_DIR.mkdir(parents=True, exist_ok=True)
            overlay = image.copy()
            if mask is not None:
                overlay[mask > 0] = (overlay[mask > 0] * 0.6 + np.array([0, 165, 255]) * 0.4).astype(np.uint8)
            if blindpath_mask is not None:
                overlay[blindpath_mask > 0] = (overlay[blindpath_mask > 0] * 0.7 + np.array([0, 255, 0]) * 0.3).astype(np.uint8)
            stamp = f"{int(time.time() * 1000)}_{self.frame_counter:06d}"
            safe_reason = "".join(ch if ch.isalnum() or ch in {"_", "-"} else "_" for ch in reason)[:48]
            cv2.imwrite(str(_CROSSWALK_DEBUG_DIR / f"{stamp}_{safe_reason}.jpg"), overlay)
        except Exception as exc:
            logger.warning("[CROSS_STREET] 保存斑马线调试图失败: %s", exc)

    def _evaluate_crosswalk_near(self, mask: np.ndarray, h: int, w: int) -> dict[str, Any]:
        if mask is None:
            return {
                "is_near": False,
                "score": 0.0,
        ...
        area_threshold = CROSSWALK_MIN_AREA_RATIO_NEAR
        score = float(area_ratio >= area_threshold) + float(bottom_ratio >= 0.80) + float(height_ratio >= 0.35)
        stable = self._is_crosswalk_candidate_stable(self._mask_bbox(mask))
        return {
            "is_near": stable and score >= CROSSWALK_NEAR_SCORE_THRESHOLD,
            "score": score,
            "stable": stable,
    ...
    def _detect_obstacles(self, image, path_mask=None):
        ...
        if NAV_DEBUG_ENABLED:
            logger.info(
                "[_detect_obstacles] 开始执行，Frame=%s, obstacle_detector=%s",
                self.frame_counter,
                "已加载" if self.obstacle_detector else "未加载",
            )
        ...
        try:
            if NAV_DEBUG_ENABLED:
                logger.info("[_detect_obstacles] 调用ObstacleDetectorClient.detect()... image.shape=%s", image.shape)
            detected_obstacles = self.obstacle_detector.detect(image, path_mask=path_mask)
            if NAV_DEBUG_ENABLED:
                logger.info("[_detect_obstacles] 返回 %s 个物体", len(detected_obstacles))
    ...
    def _draw_traffic_result(self, image: np.ndarray, traffic_result: Optional[Dict[str, Any]]) -> np.ndarray:
        if image is None or traffic_result is None:
            return image
        draw_boxes = traffic_result.get("draw_boxes") or []
        for x1, y1, x2, y2, color in draw_boxes:
            cv2.rectangle(image, (int(x1), int(y1)), (int(x2), int(y2)), color, 3)
        if not draw_boxes and traffic_result.get("bbox") is not None:
            x1, y1, x2, y2 = traffic_result["bbox"]
            raw_name = traffic_result.get("verified_light") or traffic_result.get("detected_light")
            color = TRAFFIC_BOX_COLORS.get(str(raw_name).lower(), (255, 255, 255))
            cv2.rectangle(image, (int(x1), int(y1)), (int(x2), int(y2)), color, 3)
        return image
    ...
    def process_frame(self, bgr_image: np.ndarray) -> CrossStreetResult:
        ...
        try:
            ...
            # ========== 1) 分状态执行分割 ==========
            crosswalk_mask = None
            display_crosswalk_mask = None
            blindpath_mask = None
            det_area = 0
            stable_candidate = self.last_crosswalk_candidate_stable
            ran_segmentation = False
            detect_interval = self._current_crosswalk_detection_interval()

            if self.seg_model and (
                self.frame_counter == 1
                or (self.frame_counter % detect_interval) == 0
            ):
                ran_segmentation = True
                base_thr = min(CROSSWALK_MIN_CONF, BLIND_MIN_CONF)
                detect_input = self._preprocess_crosswalk_input(bgr_image)
                detections = self.seg_model.detect(detect_input, confidence_threshold=base_thr) or []
                ...
                if cw_list:
                    ...
                    if cw_masks:
                        ...
                        if det_area < self._compute_crosswalk_min_area(h, w):
                            crosswalk_mask = None
                            det_area = 0
                ...
                if crosswalk_mask is not None and blindpath_mask is not None:
                    overlap = np.logical_and(crosswalk_mask > 0, blindpath_mask > 0)
                    overlap_ratio = float(np.count_nonzero(overlap)) / max(float(np.count_nonzero(crosswalk_mask > 0)), 1.0)
                    if overlap_ratio > 0.25:
                        cropped_crosswalk = crosswalk_mask.copy()
                        cropped_crosswalk[blindpath_mask > 0] = 0
                        if np.any(cropped_crosswalk > 0):
                            crosswalk_mask = self._postprocess_mask(cropped_crosswalk)
                
                if blindpath_mask is not None:
                    if not _looks_like_blind_path(blindpath_mask, crosswalk_mask, h, w):
                        blindpath_mask = None

                stable_candidate = self._record_crosswalk_candidate(crosswalk_mask)
                if DETECTION_DEBUG_ENABLED and (crosswalk_mask is None or not stable_candidate):
                    debug_reason = "candidate_missing" if crosswalk_mask is None else "candidate_unstable"
                    self._save_crosswalk_debug(bgr_image, crosswalk_mask, blindpath_mask, debug_reason)
                        ...
                        if p1 is not None and st is not None:
                            ...
                            if good_new is not None and len(good_new) >= TRACK_MIN_POINTS:
                                ...
                                if tracked_mask is not None and int(tracked_mask.sum()) >= (0.3 * (self.prev_mask.sum() if self.prev_mask is not None else 1)):
                                    crosswalk_mask = self._postprocess_mask(tracked_mask)
                                    used_tracking = True
                                    stable_candidate = self.last_crosswalk_candidate_stable
            ...
            if crosswalk_mask is not None:
                ...
                display_crosswalk_mask = (self.prev_mask_float > 0.5).astype(np.uint8)
                display_crosswalk_mask = self._postprocess_mask(display_crosswalk_mask)
            else:
                display_crosswalk_mask = None
            ...
            draw_crosswalk_mask = display_crosswalk_mask if display_crosswalk_mask is not None else crosswalk_mask
            if draw_crosswalk_mask is not None:
                self.crosswalk_detected = True
                # 使用与盲道模式相同的橙色：BGR(0, 165, 255)，只在掩码区域混合
                mask_area = (draw_crosswalk_mask > 0).astype(bool)
            ...
            if self.state == STATE_SEEKING:
                # 阶段1：寻找并对准远处的斑马线
                if crosswalk_mask is not None:
                    near_info = self._evaluate_crosswalk_near(crosswalk_mask, h, w)
                    self.last_crosswalk_switch_reason = "candidate_stable" if near_info["stable"] else "candidate_unstable"
                    if ran_segmentation and NAV_DEBUG_ENABLED:
                        logger.info(
                            "[CROSS_STREET] candidate=%s stable=%s near_score=%.1f state_switch_reason=%s",
                            1,
                            near_info["stable"],
                            near_info["score"],
                            self.last_crosswalk_switch_reason or "none",
                        )
                    
                    if near_info["is_near"]:
                        # 斑马线已到跟前，切换到红绿灯判定
                        self.state = STATE_WAIT_LIGHT
                        self.last_crosswalk_switch_reason = "near_score"
                        guidance_text = "斑马线已在跟前，进入红绿灯判定模式"
                        self.last_seeking_guidance = ""  # 重置节流状态
                        if DETECTION_DEBUG_ENABLED:
                            self._save_crosswalk_debug(bgr_image, crosswalk_mask, blindpath_mask, "switch_wait_light")
                    else:
                        # 远距离对准引导（使用更宽松的阈值）
                    ...
                    self.last_crosswalk_switch_reason = "no_candidate"
                    # 【移除左上角文字，改为右上角数据面板】
                    frame_visualizations.append({
                        "type": "data_panel",
                        "data": {
                ...
                if TRAFFIC_LIGHT_AVAILABLE and trafficlight_detection:
                    try:
                        # 红绿灯检测使用原始帧，避免斑马线/盲道叠色污染颜色和模型输入。
                        result = trafficlight_detection.process_single_frame(bgr_image)
                        annotated = self._draw_traffic_result(annotated, result)
                        
                        if result and 'stable_light' in result:
                            stable_light = result['stable_light']
                            traffic_stats = result.get('stats', {})
                                ...
                                if stable_light == 'stop':
                                    ...
                                    frame_visualizations.append({
                                        ...
                                        "data": {
                                            "状态": "红绿灯判定",
                                            "检测": "红灯，请等待",
                                            "候选": f"{traffic_stats.get('candidate', 0)}/{traffic_stats.get('verified', 0)}",
                                        },
                                        "position": (w - 180, 20)
                                    })
                                    # 红灯状态播报（节流）
                ...
                if TRAFFIC_LIGHT_AVAILABLE and trafficlight_detection:
                    try:
                        ...
                        if result and 'stable_light' in result:
                            stable_light = result['stable_light']
                            verified_light = result.get('verified_light')
                            if verified_light == 'countdown_go' or (stable_light == 'go' and verified_light == 'countdown_go'):
                                # 绿灯倒计时，播报警告（节流）
                                if current_time - self.last_guide_time > 2.0:
...
class YOLOModelWrapper:
    ...
    def detect(self, image, confidence_threshold=0.25):
        ...
        try:
            predict_kwargs = _build_seg_predict_kwargs(confidence_threshold)
