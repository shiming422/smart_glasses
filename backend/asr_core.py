...
class ASRCallback:
    ...
    def __init__(
        ...
        is_input_guard_active_fn: Callable[[], bool],
    ...
    def _handle(self, event: Any):
        ...
        playing_now = False
        try:
            playing_now = bool(self._is_playing())
        except Exception:
            playing_now = False
        guard_active = False
        try:
            guard_active = bool(self._is_input_guard_active())
        except Exception:
            guard_active = False
        if playing_now or guard_active:
            # If any piece of this sentence arrives during playback, mark it tainted.
            self._segment_tainted_by_tts = True

        # ---------- ① 热词优先：命中就全清零并短路，绝不送 LLM ----------
        if not self._segment_tainted_by_tts and not self._hot_interrupted and self._has_hotword(text):
        ...
        if is_end is True:
            ...
            final_playing_now = False
            try:
                final_playing_now = bool(self._is_playing())
            except Exception:
                final_playing_now = False
            final_guard_active = False
            try:
                final_guard_active = bool(self._is_input_guard_active())
            except Exception:
                final_guard_active = False
            if final_guard_active:
                self._segment_tainted_by_tts = True

            if self._segment_tainted_by_tts and final_text:
                print(f"[ASR DROP] tainted final dropped: '{_shorten(final_text)}'", flush=True)
            elif (not final_playing_now) and (not final_guard_active) and final_text:
            ...
            self._segment_tainted_by_tts = False
