...
class CrosswalkAwarenessMonitor:
    ...
    def __init__(self):
        # 面积阈值（固定锚点）
        self.THRESHOLDS = {
            'discover': _env_float("AIGLASS_CROSSWALK_AWARE_DISCOVER", 0.006),
            'approaching': _env_float("AIGLASS_CROSSWALK_AWARE_APPROACHING", 0.04),
            'near': _env_float("AIGLASS_CROSSWALK_AWARE_NEAR", 0.12),
            'arrival': _env_float("AIGLASS_CROSSWALK_AWARE_ARRIVAL", 0.20),
        }
        ...
        self.stage_history = deque(maxlen=30)
    ...
    def process_frame(self, crosswalk_mask, blind_path_mask=None) -> Optional[Dict[str, Any]]:
        ...
        self.area_history.append({
            ...
            'time': current_time,
            'stage': self._get_stage(area_ratio),
        })
        self.stage_history.append(self._get_stage(area_ratio))
    ...
    def _generate_guidance(self, area_ratio, center_x_ratio, center_y_ratio, 
        ...
        # 降级处理：如果从到达状态面积减小
        if self.in_arrival_state and area_ratio < self.THRESHOLDS['arrival']:
    ...
    def _get_stage(self, area_ratio: float) -> str:
        if area_ratio >= self.THRESHOLDS['arrival']:
            return 'arrival'
        if area_ratio >= self.THRESHOLDS['near']:
            return 'near'
        if area_ratio >= self.THRESHOLDS['approaching']:
            return 'approaching'
        if area_ratio >= self.THRESHOLDS['discover']:
            return 'discover'
        return 'none'
    
    def _is_area_stable(self, area_ratio, stability_frames=5) -> bool:
        """检查阶段是否稳定（避免曝光变化导致忽有忽无）"""
        if len(self.stage_history) < stability_frames:
            return True  # 初始阶段，认为稳定
        current_stage = self._get_stage(area_ratio)
        recent_stages = list(self.stage_history)[-stability_frames:]
        same_stage = sum(1 for stage in recent_stages if stage == current_stage)
        return same_stage >= 3
